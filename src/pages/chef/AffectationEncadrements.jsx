...
const AffectationEncadrements = () => {
  ...
  const etudiantsFiltres = useMemo(() => {
    ...
    return filtres.sort((a, b) => {
      ...
      const compareNom = nomA.localeCompare(nomB, 'fr', { sensitivity: 'base' });
  ...
  const enseignantsFiltres = useMemo(() => {
    // Filtrer d'abord
    let filtres = enseignants;

    if (searchEnseignant && searchEnseignant.trim() !== '') {
      filtres = enseignants.filter(ens => {
    ...
    // Trier par ordre alphabétique (nom puis prénom)
    return filtres.sort((a, b) => {
      const nomA = a?.nom_ens || a?.Nom_Ens || '';
      const nomB = b?.nom_ens || b?.Nom_Ens || '';
      const prenomA = a?.prenom_ens || a?.Prenom_Ens || '';
      const prenomB = b?.prenom_ens || b?.Prenom_Ens || '';
  ...
  try {
    ...
    // On utilise les données déjà présentes dans le state 'etudiants'
    await exportAffectationsEtudiantsPDF(etudiants);
    toast.success('PDF des étudiants généré avec succès');
  } catch (error) {
    console.error('Erreur export PDF:', error);
    toast.error('Erreur lors de la génération locale du PDF');
  }
};
...
const handleExportPDFEncadreurs = async () => {
  try {
    toast.info('Génération du PDF en cours...');
    // On utilise les données déjà présentes dans le state 'enseignants'
    await exportStatsEncadreursPDF(enseignants);
    toast.success('PDF des encadreurs généré avec succès');
  } catch (error) {
    console.error('Erreur export PDF:', error);
    toast.error('Erreur lors de la génération locale du PDF');
  }
